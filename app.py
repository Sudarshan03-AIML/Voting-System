...
def result1():
    ...
    # Fetch the votes for each nominee
    cursor.execute('''
    ...
    total_votes = sum(vote_count for _, vote_count in result)
    ...
    # Pass total_votes to the template
    return render_template('result1.html', result=result, winner=winner, total_votes=total_votes)
...
def voted_list():
    ...
    return render_template('voted_list.html', table_data=table_data)




@app.route('/nominee_form', methods=['GET', 'POST'])
def nominee_form():
    if request.method == 'POST':
